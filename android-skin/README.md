...
**Skin Tracker** is a derivative of the native **Keto Tracker** Android app
(`../android/`), mirrored into its own module (package `com.skintracker`,
app id `com.skintracker`) so it can evolve independently. Its intended
purpose is to help patients with skin issues log symptoms day-to-day, while
retaining the keto app's meal-tracking workflow.

> **Current state:** this is a faithful *mirror + rebrand* of the Keto Tracker
> native app. The application identity (package, app name, launcher label,
> theme) has been rebranded to Skin Tracker, but the underlying domain model
> and wizard are still the keto data model. Repurposing the tracked fields to
> skin symptoms is a follow-up step.

The sections below describe the inherited architecture, which is shared with
the Keto Tracker native app it was copied from.

---

**Origin (Keto Tracker v2.0)** — a ground-up **native rewrite in Kotlin +
Jetpack Compose** of the single-file PWA at the repo root (`index.html`). The
goal was feature parity with the PWA first, then to exceed it using native
capabilities the browser can't offer (CameraX, unrestricted local storage via
Room, background work, richer notifications, etc). Every screen, theme, and
interaction was ported field-for-field from `index.html`.
